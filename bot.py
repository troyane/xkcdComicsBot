...
def getRequestedComics(update, request):
    comics = ''
    if request == states.S_NEWEST:
        comics = Comics.getLatestComics()
    elif request == states.S_OLDEST:
        comics = Comics.getOldestComics()
    elif request == states.S_RANDOM:
        comics = Comics.getRandomComics()
    elif request == states.S_PREV:
        ...
        if Comics.isComicsAvailable(newNum):
            comics = Comics.getComicsByNumber(newNum)
        else:
            user_friendly_warning = "There is no comics with #{}. If you think it is problem -- write to authors. \n" \
                                    "Just in case, XKCD updates every Monday, Wednesday, and Friday.".format(currentNum)
            raise ValueError('No requested comics.',  user_friendly_warning)
    elif request == states.S_NEXT:
        currentNum = getUserComicsNumber(update)
        newNum = currentNum + 1
        if Comics.isComicsAvailable(newNum):
            comics = Comics.getComicsByNumber(newNum)
        else:
            user_friendly_warning = "Unfortunately we can't get your {} comics. Probably it will be ready soon. \n"  \
                                    "XKCD updates every Monday, Wednesday, and Friday.".format(currentNum)
            raise ValueError('No requested comics (yet).',  user_friendly_warning)
    return comics

def sendMessaageAboutError(bot, cur_chat_id, errText):
    """ Sends error message with funny picture. """
    bot.send_photo(chat_id=cur_chat_id, disable_notification=True, photo=open(QuestionImage, 'rb'))
    bot.send_message(chat_id=cur_chat_id, disable_notification=False, parse_mode='Markdown',
                     disable_web_page_preview=True, text=errText)
...
def onButtonClicked(bot, update):
    ...
    if query.data == states.S_NUMBER:
        ...
        bot.send_message(chat_id=cur_chat_id, disable_notification=False, parse_mode='Markdown', text=text2send)
        setUserState(update, states.S_NUMBER)
    else:
        try:
            comics = getRequestedComics(update, query.data)
        except ValueError as err:
            sendMessaageAboutError(bot, cur_chat_id, err.args[0])

        if comics != '':
            if 'num' in comics:
                num = comics['num']
                prepared_comics = prepareComicsToSend(comics)
                sendComics(bot, cur_chat_id, prepared_comics)
                setUsersComicsNumber(update, num)
    ...
    if state == states.S_NUMBER:
        ...
        if utils.RepresentsInt(msg):
            ...
            if Comics.isComicsAvailable(num):
                ...
                sendMessaageAboutError(bot, update.effective_chat.id, )
                user_friendly_text = "Unfortunately there is no comics with number `{}` :( \n" \
                            "Let's hope that someday there will be even `{}` comics on XKCD! \n\n" \
                            "But still you can use /menu for another requests.".format(num, num + 1)
                sendMessaageAboutError(bot, update.effective_chat.id, user_friendly_text)
                setUserState(update, states.S_START)
                return

        else:
            setUserState(update, states.S_NUMBER)
            user_friendly_text = "`{}` is not a valid integer number, you know it. \n\n" \
                        "Please, send me *only* number, without any other symbols. " \
                        "Examples of correct numbers: `13`, `42`, `666`, `1000`, ...\n\n" \
                        "Or you can use /menu as usual.".format(msg)

            sendMessaageAboutError(bot, update.effective_chat.id, user_friendly_text)
            return

    # Problem happened
    user_friendly_text = "We tried to understand your answer, but we forgot what was the question... " \
                "Hope our admins will check logs and solve this problem in newer versions of this bot. \n\n" \
                "Meanwhile you can use /menu for another requests."
    sendMessaageAboutError(bot, update.effective_chat.id, user_friendly_text)
    setUserState(update, states.S_START)
